...
def coinpage(request, coin_id):
    ...
    try:
        price = requests.get(url).json()[coin.cg_link.split('/')[5]]['usd']
    except:
        price = 'Wrong id'
    context = {
        'coin': coin,
...
def feed(request):
    ...
    posts = Post.objects.filter(coin__in=followed).order_by('-date_added')
    pcs = []
    for i in PriceDynamic.objects.all():
        data = model_to_dict(i)
        data['name'] = i.coin.name
    ...
    return render(request, 'news/feed.html', {'posts': posts, 'pcs': pcs})
